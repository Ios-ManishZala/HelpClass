...
@objc protocol DownloadProcessProtocol {
    ...
    func downloadingProgress(_ percent: Float, fileName: String, totalMB: String, currentMB: String)
...
extension DownloadManager: URLSessionDownloadDelegate {
    ...
    func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didWriteData bytesWritten: Int64, totalBytesWritten: Int64, totalBytesExpectedToWrite: Int64) {
        ...
        if let downloadUrl = downloadTask.originalRequest!.url {
            ...
            DispatchQueue.main.async { [self] in
                processDelegate?.downloadingProgress(Float(percent), fileName:  downloadUrl.lastPathComponent, totalMB: expected, currentMB: written)
            }
        }
    }
